...
test("nested packages sharing a configuration retain their own framework context", () => {
  const root = mkdtempSync(
    join(tmpdir(), "code-polishy-provider-shared-config-"),
  );
  try {
    mkdirSync(join(root, "site/node_modules/astro"), { recursive: true });
    writeFileSync(join(root, "package.json"), '{"type":"module"}');
    writeFileSync(
      join(root, "tsconfig.json"),
      '{"compilerOptions":{"strict":true,"skipLibCheck":true,"target":"ESNext","module":"ESNext","moduleResolution":"Bundler"},"include":["root.ts","site/**/*.astro"]}',
    );
    ...
    writeFileSync(
      join(root, "site/node_modules/astro/package.json"),
      '{"version":"7.2.4"}',
    );
    writeFileSync(join(root, "site/node_modules/astro/env.d.ts"), "");
    writeFileSync(
      join(root, "site/node_modules/astro/astro-jsx.d.ts"),
      "declare namespace JSX { interface IntrinsicElements { [name: string]: any } }\n",
    );
    writeFileSync(
      join(root, "site/page.astro"),
      "---\nconst value: number = 1;\n---\n<h1>{value}</h1>\n",
    );
    const response = analyze(
      requestFor(root, ["root.ts", "site/page.astro"], "typecheck"),
    );
    assert.equal(response.status, "pass", JSON.stringify(response));
    assert.deepEqual(response.coverage.unsupported, []);
