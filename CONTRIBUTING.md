...
6. Before merge, resolve the trusted merge target from an explicit target,
   ...
   user to choose recommended or full; it is the sole autonomous ordinary
   authority. Run `test --supplemental` as a separate stage when the task or
   release workflow requires local supplemental hardening. Credentialed,
...
- minor: additive checks, conditional modules, providers, or config with a
  documented adoption path;
- major: changed defaults, removed config, or broadly new blocking behavior.
