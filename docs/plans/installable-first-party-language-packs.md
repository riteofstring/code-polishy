...
Status (2026-09-22): the first executable slice owns nearest-`pyproject.toml`
static discovery, isolated Ruff format and lint, and CPython comment/docstring
facts. Ruff uses no cache, format writes return only core-authorized edits, and
four built-pack fixtures prove pass and seeded-defect outcomes. Type checking,
complexity, dead code, architecture, dependency evidence, project-version
derivation, framework contracts, and the shared conformance matrix remain open.
