...
Core supplies an authoritative governed inventory and exact selection. Static
pack discovery may group only provider-owned source from that inventory, name
contained context, identify entries, and attach bounded opaque ecosystem data.
Core validates those claims and replaces pack-private identities with
invocation-local handles before capability execution.

Generated source keeps its physical path for reads, findings, and write
protection. Its `scope.sourceContexts` mapping supplies module ownership and a
contained ecosystem context. The selected pack decides whether that context is
a package manifest, project file, module declaration, or another supported
ecosystem boundary; core does not reconstruct that shape.
