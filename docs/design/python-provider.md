...
methods only on the configured type and proven subclasses. The adapter never
runs a reduced analysis and presents it as complete.
...
TypedDict and framework retention, type-contract attributes, annotated fields,
and decorated members, dynamic references, external attributes, cross-module
