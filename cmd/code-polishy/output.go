...
func reportOutputCommand(command string) bool {
	switch command {
	case "change-boundary", "doctor", "gate", "checkpoint-gate", "merge-gate", "check", "architecture", "architecture-review", "design-context", "format", "fix", "size", "test", "test-plan", "test-levels", "verify", "supply-chain", "dependency-review", "artifact-security":
		return true
	default:
		return false
	}
