...
var commandHelpPages = []commandHelpPage{
	...
	},
	{
		...
		syntax: []string{
			"code-polishy pack catalog --catalog PATH --sha256 DIGEST [--format human|json]",
			...
			"code-polishy pack validate --kind manifest|request|response --input PATH [--request PATH]",
			"code-polishy pack list [--format human|json]",
			"code-polishy pack migration plan --catalog PATH --sha256 DIGEST --select NAME@VERSION...",
			"code-polishy pack migration apply --plan PATH",
			"code-polishy pack migration rollback --plan PATH",
			"code-polishy pack conformance --ledger PATH --reference PATH --candidate PATH [--reference-policy-root PATH] [--candidate-policy-root PATH] [--candidate-pack PATH...]",
			"code-polishy pack root",
		},
		selectors: []string{
			"catalog authenticates and displays one bounded local official-pack catalog.",
			"install validates and atomically installs one local source directory or one exact authenticated official catalog entry.",
			"update installs one exact authenticated candidate and reports its authority and inventory without changing repository selection.",
			"remove deletes one exact installed identity; --digest is required when a name and version have multiple installed trees.",
			"verify runs the source directory's declared conformance fixtures without installing it.",
			"validate applies the shipped schema and production semantic validator without executing pack code; response validation requires its exact request.",
			"list distinguishes installed, selected, missing, incompatible, and corrupt exact identities.",
			"conformance runs identical repositories through exact reference and candidate executables and authenticated policy roots, optionally installing exact local pack sources only for the isolated candidate lane, and emits one machine-readable differential report.",
			"migration plans install and verify an explicit complete replacement set, compare coverage and diagnostics, and preserve a rollback copy before any policy cutover.",
			"migration apply atomically replaces only pack policy; rollback restores the exact prior configuration. Neither operation changes the engine lock.",
			"root prints the effective local pack storage root.",
		},
		sideEffects: []string{"catalog and list only read local authenticated state; install and update write an immutable content-addressed user-data directory without executing pack code or changing repository policy; remove deletes only one explicit local pack identity and never edits repositories; verify executes declared local adapters; migration plan installs and verifies only named catalog artifacts and writes managed evidence without selecting them; migration apply or rollback atomically changes only the repository configuration; conformance executes both named binaries in disposable repositories and installs candidate pack sources only into its temporary data home; root only reads platform configuration."},
		exits:       []string{"0 requested action completed or a migration is ready", "1 conformance behavior differed or a migration preview is blocked", "2 invalid usage, unsafe pack, stale evidence, unavailable integration, or operational failure"},
		examples:    []string{"code-polishy pack catalog --catalog ./catalog.json --sha256 DIGEST", "code-polishy pack install --official shell@1.0.0 --catalog ./catalog.json --sha256 DIGEST", "code-polishy pack update shell --to 1.1.0 --catalog ./catalog.json --sha256 DIGEST", "code-polishy pack migration plan --catalog ./catalog.json --sha256 DIGEST --select shell@1.0.0 --select python@1.0.0", "code-polishy pack list", "code-polishy pack verify --source ./code-polishy-rust", "code-polishy pack conformance --ledger ./tests/language-conformance/ledger.json --reference ./reference/code-polishy --candidate ./candidate/code-polishy"},
	},
	{
		name:    "agents",
	...
	},
	{
		...
		summary: "Manage selected-review intent and evidence, or complete a behavior and final-state review.",
		syntax: []string{
			"code-polishy behavior-review capture-intent --intent-file PATH|- [--feature NAME...] [--format human|json]",
			"code-polishy behavior-review cleanup [--format human|json]",
		...
		selectors: []string{
			"capture-intent requires one regular-file path or - for standard input and a clean committed base for the original request; later artifact-affecting corrections may bind a dirty candidate.",
			...
			"capture-intent, cleanup, and status always confirm canonical state on stdout, including in pipes; --format json emits one behavior-review/v1 document.",
			"cleanup removes the complete local behavior-review artifact directory. Run it only after no selected gate or evidence transfer still needs those artifacts.",
		...
		sideEffects: []string{"capture-intent and require append to the managed intent journal; cleanup removes the complete managed behavior-review directory; status only reads review state; prepare writes a review packet; finalize writes the corresponding review receipt."},
		...
		examples: []string{
			"code-polishy behavior-review capture-intent --intent-file - --feature checkout --feature search",
			"code-polishy behavior-review cleanup",
	...
	},
	{
		...
		selectors: []string{
			"--strict enables stricter environment checks and reports every installed or selected pack as selected, installed, missing, incompatible, or corrupt.",
		},
