...
var commandHelpPages = []commandHelpPage{
	...
	},
	{
		...
		sideEffects: []string{"catalog and list only read local authenticated state; install and update write an immutable content-addressed user-data directory without executing pack code or changing repository policy; remove deletes only one explicit local pack identity and never edits repositories; verify executes declared local adapters; conformance executes both named binaries in disposable repositories; root only reads platform configuration."},
		...
		examples:    []string{"code-polishy pack catalog --catalog ./catalog.json --sha256 DIGEST", "code-polishy pack install --official shell@1.0.0 --catalog ./catalog.json --sha256 DIGEST", "code-polishy pack update shell --to 1.1.0 --catalog ./catalog.json --sha256 DIGEST", "code-polishy pack list", "code-polishy pack verify --source ./code-polishy-rust", "code-polishy pack conformance --ledger ./tests/language-conformance/ledger.json --reference ./reference/code-polishy --candidate ./candidate/code-polishy"},
	},
	{
		name:    "agents",
