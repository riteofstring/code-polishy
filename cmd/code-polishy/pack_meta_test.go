...
func TestPackHelpAndRootWorkBeforeRepositoryInitialization(t *testing.T) {
	...
	if status != 0 || stderr != "" || !strings.Contains(stdout, "code-polishy pack install --source PATH") || !strings.Contains(stdout, "code-polishy pack install --official NAME@VERSION") || !strings.Contains(stdout, "code-polishy pack list") || !strings.Contains(stdout, "code-polishy pack migration plan") || !strings.Contains(stdout, "code-polishy pack conformance --ledger PATH --reference PATH --candidate PATH") {
...
func TestPackRejectsIncompleteAndUnknownActionsBeforeReadingSource(t *testing.T) {
	for _, arguments := range [][]string{{"pack"}, {"pack", "catalog"}, {"pack", "install"}, {"pack", "update"}, {"pack", "remove"}, {"pack", "verify"}, {"pack", "list", "extra"}, {"pack", "migration"}, {"pack", "migration", "plan"}, {"pack", "migration", "apply"}, {"pack", "migration", "rollback"}, {"pack", "conformance"}, {"pack", "conformance", "--ledger", "ledger.json", "--reference", "reference"}, {"pack", "root", "extra"}, {"pack", "unknown"}} {
