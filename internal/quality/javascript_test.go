...
func TestJavaScriptFormatExemptsGeneratedExecutableSource(t *testing.T) {
	t.Parallel()
	repo := qualityRepository(t)
...
func TestGeneratedOnlyJavaScriptFormattingDoesNotInvokeTheFormatter(t *testing.T) {
	t.Parallel()
	repo := qualityRepository(t)
	path := "app/client.generated.ts"
	original := "export const value={  answer:42}\n"
	writeQualityFile(t, repo.Root, path, original)
	repo.PolicyRoot, _ = fakeFileBundle(t, `{"changed":[],"unsupported":[]}`)
	for _, write := range []bool{false, true} {
		if findings := javascriptFormat(t.Context(), repo, []string{path}, write); len(findings) != 0 {
			t.Fatalf("generated-only format = %+v", findings)
		}
	}
	data, err := os.ReadFile(filepath.Join(repo.Root, path))
	if err != nil || string(data) != original {
		t.Fatalf("generated bytes = %q, %v", data, err)
	}
	repo.PolicyRoot = t.TempDir()
	if findings := JavaScriptFormatFindings(t.Context(), repo, []string{path}); len(findings) != 0 {
		t.Fatalf("style exemption required an unused formatter: %+v", findings)
	}
}
