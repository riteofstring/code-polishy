...
func TestProviderEvidenceBindsDiscoveredAssetLinkResolution(t *testing.T) {
	root := t.TempDir()
	writeTestFile(t, root, "src/main.js", "export const image = './picture.png';\n", 0o600)
	writeTestFile(t, root, "assets/first.png", "same", 0o600)
	writeTestFile(t, root, "assets/second.png", "same", 0o600)
	link := filepath.Join(root, "src/picture.png")
	if err := os.Symlink("../assets/first.png", link); err != nil {
		t.Skipf("symlink unavailable: %v", err)
	}
	config := policy.Config{Modules: []policy.Module{{Name: "application", Paths: []string{"assets/**", "src/**"}}}, ModuleByName: map[string]int{"application": 0}}
	repo, err := repository.Open(root, root, config)
	if err != nil {
		t.Fatal(err)
	}
	request := Request{Files: []string{"src/main.js"}, DiagnosticFiles: []string{"src/main.js"}, Capability: "architecture"}
	if err := prepareInputs(repo, &request); err != nil {
		t.Fatal(err)
	}
	if !slices.ContainsFunc(request.Context, func(input InputFile) bool { return input.Path == "src/picture.png" }) {
		t.Fatal("provider context omitted a discovered asset link")
	}
	response := Response{Status: "pass", Inputs: request.Context}
	if err := verifyAnalysisInputs(repo, request, response); err != nil {
		t.Fatal(err)
	...
	if err := os.Symlink("../assets/second.png", link); err != nil {
		t.Fatal(err)
	}
	if err := verifyAnalysisInputs(repo, request, response); err == nil {
		t.Fatal("retargeted asset link received valid provider evidence")
	}
}
