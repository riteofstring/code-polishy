...
func validProviderIdentity(input FactInput, provider ProviderInput) bool {
	return input.Analyzer == "pack" && input.Protocol == "code-polishy-pack/v4" && providerIdentifier.MatchString(provider.Name) && providerVersion.MatchString(provider.Version)
}
