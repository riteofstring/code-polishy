...
func (engine *Engine) normalizeFinding(finding policy.Finding, requested *repository.RequestedSelection) policy.Finding {
	...
	finding = policy.NormalizeFinding(finding)
	retainRemediationSelection(&finding, requested)
	return finding
}

func retainRemediationSelection(finding *policy.Finding, requested *repository.RequestedSelection) {
	command := finding.Remediation.NextCommand
	if requested == nil || command == nil || len(command.Argv) < 3 || command.Argv[0] != "code-polishy" ||
		!slices.Contains([]string{"check", "architecture"}, command.Argv[1]) {
		return
	}
	baseline := *finding
	baseline.Remediation = policy.FindingRemediation{}
	baseline = policy.NormalizeFinding(baseline)
	if finding.Remediation.Summary != baseline.Remediation.Summary || !slices.Equal(command.Argv, baseline.Remediation.NextCommand.Argv) {
		return
	}
	if selector := remediationSelector(*finding, *requested); len(selector) != 0 {
		finding.Remediation.NextCommand = &policy.FindingCommand{Argv: append(slices.Clone(command.Argv[:2]), selector...), Cwd: command.Cwd}
	}
}

func remediationSelector(finding policy.Finding, requested repository.RequestedSelection) []string {
	if requested.Mode == "module" && slices.Contains(requested.Modules, finding.Module) {
		return []string{"--module", finding.Module}
	}
	if requested.Mode == "files" {
		for _, operand := range requested.Operands {
			if finding.Path == operand || strings.HasPrefix(finding.Path, strings.TrimSuffix(operand, "/")+"/") {
				return []string{"--files", operand}
			}
		}
	}
	return nil
