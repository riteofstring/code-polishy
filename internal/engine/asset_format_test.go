...
func TestFormatDoesNotPassAssetFileLinksToSourceFormatters(t *testing.T) {
	root := t.TempDir()
	original := []byte("const value=1")
	writeEngineFile(t, root, "assets/picture.png", string(original), 0o600)
	if err := os.Symlink("assets/picture.png", filepath.Join(root, "source.js")); err != nil {
		t.Skipf("symlink unavailable: %v", err)
	}
	config := policy.Config{Quality: policy.EffectiveQuality(policy.Quality{}), Modules: []policy.Module{{Name: "application", Paths: []string{"assets/**", "source.js"}}}, ModuleByName: map[string]int{"application": 0}}
	repo, err := repository.Open(root, enginePolicyRoot(t), config)
	if err != nil {
		t.Fatal(err)
	}
	engine := Engine{Repository: repo, Runner: runner.OSRunner{}}
	report := engine.Format(t.Context(), repository.Selection{Files: []string{"source.js"}, Candidate: repository.CandidateDelta{AddedOrModified: []string{"source.js"}}})
	if HasFindings(report) || report.Formatting == nil || report.Formatting.Protected != 1 {
		t.Fatalf("format failed: findings=%+v formatting=%+v", report.Findings, report.Formatting)
	}
	data, err := os.ReadFile(filepath.Join(root, "assets/picture.png"))
	if err != nil || !slices.Equal(data, original) {
		t.Fatalf("protected asset changed: %q %v", data, err)
	}
}
